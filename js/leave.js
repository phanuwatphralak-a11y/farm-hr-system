...
const LEAVE = {
        ...
        <td>
          <div class="action-btns">
            ${l.status === "รอการอนุมัติ" ? `
              <button class="btn-sm btn-sm-approve" onclick="LEAVE.approve(${l._row})" title="อนุมัติ"><i class="fas fa-check"></i></button>
              <button class="btn-sm btn-sm-reject" onclick="LEAVE.reject(${l._row})" title="ปฏิเสธ"><i class="fas fa-times"></i></button>
            ` : ""}
            <button class="btn-sm btn-sm-del" onclick="LEAVE.confirmDelete(${l._row})" title="ลบ"><i class="fas fa-trash"></i></button>
          </div>
        </td>
      </tr>`).join("");
  },
  ...
  statusBadge(status) {
    const map = { "รอการอนุมัติ": "badge-orange", "อนุมัติ": "badge-green", "ปฏิเสธ": "badge-red" };
  ...
  renderQuota() {
    ...
    if (!emps.length) { container.innerHTML = `<p class="empty-msg">ไม่มีข้อมูลพนักงาน</p>`; return; }
    ...
    STATE.leave
      ...
      .forEach(l => {
        ...
        usedMap[l.emp_id][l.leave_type] = (usedMap[l.emp_id][l.leave_type] || 0) + (parseInt(l.days) || 1);
      });
    ...
    container.innerHTML = `
      <div class="quota-year-label"><i class="fas fa-calendar-check"></i> โควต้าวันลา ปี ${thYear}</div>
      <div class="card">
        <div class="quota-table-wrap">
          <table class="data-table">
            <thead>
              <tr>
                <th>พนักงาน</th>
                ${types.map(t => { const q = this.QUOTA[t]; return `<th>${t}<br><small>${q !== null ? q + " วัน/ปี" : "ไม่จำกัด"}</small></th>`; }).join("")}
              </tr>
            </thead>
            <tbody>
              ${emps.map(e => {
                ...
                return `<tr>
                  <td><strong>${e.emp_id}</strong><br><small style="color:var(--text-muted)">${e.first_name} ${e.last_name}</small></td>
                  ${types.map(t => {
                    const quota = this.QUOTA[t];
                    const u = used[t] || 0;
                    if (quota === null) return `<td><span style="color:var(--text-muted)">${u} วัน</span></td>`;
                    const pct = Math.min(100, Math.round(u / quota * 100));
                    const over = u > quota;
                    return `<td>
                      <div class="quota-cell">
                        <span class="${over ? "text-danger" : ""}">${u}/${quota} วัน${quota - u > 0 ? ` <small style="color:var(--text-muted)">(เหลือ ${quota-u})</small>` : ""}</span>
                        <div class="quota-bar"><div class="quota-fill${over ? " over" : ""}" style="width:${pct}%"></div></div>
                      </div></td>`;
                  }).join("")}
                </tr>`;
              }).join("")}
  ...
  async save(form) {
    ...
    const obj = {
      leave_id: this.genId(empId), request_date: fd.get("request_date"),
      emp_id: empId, emp_name: empName, leave_type: fd.get("leave_type"),
      start_date: start, end_date: end, days: this.calcDays(start, end),
      reason: fd.get("reason"), status: "รอการอนุมัติ",
      approved_by: "", approved_date: "", notes: fd.get("notes")
    };
    showLoading(true);
    try {
      ...
      await this.load(document.getElementById("leave-year-filter")?.value || "");
  ...
  async approve(rowIndex) {
    ...
    if (!item) return;
    if (!confirm(`อนุมัติการลา?\nพนักงาน: ${item.emp_name}\nประเภท: ${item.leave_type} (${item.days} วัน)\nวันที่: ${formatDate(item.start_date)} – ${formatDate(item.end_date)}`)) return;
    const updated = { ...item, status: "อนุมัติ", approved_by: STATE.currentUser?.name || "admin", approved_date: new Date().toISOString().slice(0,10) };
    showLoading(true);
    try {
  ...
  async reject(rowIndex) {
    ...
    const updated = { ...item, status: "ปฏิเสธ", approved_by: STATE.currentUser?.name || "admin", approved_date: new Date().toISOString().slice(0,10), notes: reason ? `[ปฏิเสธ] ${reason}` : "[ปฏิเสธ]" };
    showLoading(true);
    try {
      await API.update(CONFIG.SHEETS.LEAVE, rowIndex, this.objToRow(updated));
  ...
  switchTab(tab) {
    ...
    document.querySelectorAll(".leave-tab-btn").forEach(b => b.classList.toggle("active", b.dataset.tab === tab));
